...
export async function GET(req: NextRequest) {
    ...
    for (const wf of workflows) {
        try {
            ...
            const chunks = chunkArray(checkIds, 150);
            ...
            const processedIds = new Set(
                existingExecs.map(e => isLeadsSource ? e.lead_id : e.customer_id).filter(Boolean)
            );

            // Also track seen IDs in this run to avoid duplicates within the daily cron list itself
