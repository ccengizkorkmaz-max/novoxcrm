...
import { PhoneCall, UserCheck, Lock, RefreshCw, LogOut, CheckCircle2 } from 'lucide-react'
import { authenticate, logout, getBunyaminLeads } from './actions'

export default function BunyaminClient({ initialAuthed }: { initialAuthed: boolean }) {
    ...
    const loadLeads = async () => {
        ...
        const res = await getBunyaminLeads()
        if (res.success) {
            setLeads(res.leads)
        } else {
            ...
            <div className="min-h-screen bg-slate-900 text-white flex flex-col justify-center items-center p-4">
                <div className="w-full max-w-sm bg-slate-800/90 border border-slate-700 rounded-3xl p-6 shadow-2xl space-y-6">
                    <div className="text-center space-y-2">
                        ...
                        <h1 className="text-xl font-black tracking-tight">Bünyamin Saraç</h1>
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            ...
            <div className="sticky top-0 z-40 bg-white border-b border-slate-200 px-4 py-3 shadow-sm flex items-center justify-between">
                <div>
                    <h1 className="text-lg font-bold text-slate-900">Bünyamin Saraç</h1>
