        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-3">
                <Card className="bg-blue-50/50 border-blue-100">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-blue-600">{t('stats.pendingApps')}</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="bg-green-50/50 border-green-100">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-green-600">{t('stats.activeBrokers')}</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="flex items-center gap-2">
                ...
                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">{t('stats.totalApps')}</CardTitle>
                    </CardHeader>
                    <CardContent>
            ...
            <Tabs defaultValue="applications" className="w-full">
                <TabsList className="grid w-full grid-cols-2 max-w-[400px]">
                    ...
                    <TabsTrigger value="brokers">{t('tabs.brokers')}</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="applications" className="mt-6">
                    <Card>
                        <CardHeader>
                            <CardTitle>{t('list.applicationsTitle')}</CardTitle>
                            <CardDescription>{t('list.applicationsDesc')}</CardDescription>
                        </CardHeader>
                        <CardContent className="p-0">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead>{t('table.headers.brokerInfo')}</TableHead>
                                            ...
                                            <TableRow key={app.id}>
                                                ...
                                                <TableCell>
                                                    <div className="flex items-center gap-1 text-sm" suppressHydrationWarning>
                                                        ...
                                                        {new Date(app.created_at).toLocaleDateString(formatLocale)}
                                                    </div>
                                                </TableCell>
                                                <TableCell>
                                                ...
                                                <TableCell className="text-right">
                                                    {app.status === 'Pending' ? (
                                                        ...
                                                        <span className="text-xs text-muted-foreground italic" suppressHydrationWarning>
                                                            {t('table.processedOn', { date: new Date(app.processed_at).toLocaleDateString(formatLocale) })}
                                                        </span>
                                                    )}
                                                </TableCell>
                                            </TableRow>
                ...
                <TabsContent value="brokers" className="mt-6">
                    <Card>
                        <CardHeader>
                            <CardTitle>{t('list.brokersTitle')}</CardTitle>
                            <CardDescription>{t('list.brokersDesc')}</CardDescription>
                        </CardHeader>
                        <CardContent className="p-0">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead>{t('table.headers.status')}</TableHead>
                                        <TableHead>{t('table.headers.contact')}</TableHead>
                                        <TableHead>{t('table.headers.signupDate')}</TableHead>
                                        <TableHead className="text-right">{t('table.headers.actions')}</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                                ...
                                                <TableRow key={broker.id} className={!isActive ? 'bg-slate-50 opacity-60' : ''}>
                                                    ...
                                                    <TableCell>
                                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                                            {isActive ? t('table.status.Active') : t('table.status.Passive')}
                                                        </span>
                                                    </TableCell>
                                                    <TableCell>
                                                        <div className="flex flex-col gap-1">
                                                    ...
                                                    <TableCell>
                                                        <span className="text-xs text-muted-foreground" suppressHydrationWarning>
                                                            {new Date(broker.created_at).toLocaleDateString(formatLocale)}
                                                        </span>
                                                    </TableCell>
                                                    <TableCell className="text-right">
