...
export default function BrokerApplicationActions({ applicationId }: { applicationId: string }) {
    ...
    async function handleAction(status: 'Approved' | 'Rejected') {
        ...
        if (res.error) {
            ...
            toast.success(status === 'Approved' ? t('actions.successApprove') : t('actions.successReject'))
    ...
    return (
        <div className="flex gap-2">
            <AlertDialog>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>{t('actions.confirmRejectTitle')}</AlertDialogTitle>
                        <AlertDialogDescription>
                            {t('actions.confirmRejectDesc')}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
