'use client'

import { useState } from 'react'
import { useRouter } from 'next/navigation'
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { Textarea } from "@/components/ui/textarea"
import { Building2, Loader2, Info, BadgePercent, CreditCard, Layers } from 'lucide-react'
import { createCommissionModel } from '@/app/broker/actions'
import { toast } from 'sonner'
import { useTranslations } from 'next-intl'

interface Project {
    id: string
    name: string
}

export default function CommissionModelForm({ projects, tenantId }: { projects: Project[], tenantId: string }) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [type, setType] = useState('Flat %')
    const t = useTranslations('CommissionSettings')

    async function handleSubmit(formData: FormData) {
        setLoading(true)

        const data = {
            tenant_id: tenantId,
            project_id: formData.get('project_id') as string,
            name: formData.get('name') as string,
            type: formData.get('type') as string, // Flat %, Unit Based %, Project Based %, Tiered
            value: Number(formData.get('value')),
            currency: formData.get('currency') as string || 'TRY',
            payable_stage: formData.get('payable_stage') as string,
            payment_terms: formData.get('payment_terms') as string || 'Net', // Net/Brut
            start_date: formData.get('start_date') ? formData.get('start_date') as string : null,
            end_date: formData.get('end_date') ? formData.get('end_date') as string : null,
        }

        if (!data.name || !data.value) {
            toast.error(t('form.errors.required'))
            setLoading(false)
            return
        }

        if (data.value < 0) {
            toast.error(t('form.errors.negative'))
            setLoading(false)
            return
        }

        const isPercentage = data.type.includes('%') || data.type === 'Tiered'
        if (isPercentage && data.value > 100) {
            toast.error(t('form.errors.percentage'))
            setLoading(false)
            return
        }

        const result = await createCommissionModel(data)

        if (result.error) {
            toast.error(result.error)
            setLoading(false)
        } else {
            toast.success(t('form.success'))
            router.push('/admin/broker-leads/commission-settings')
        }
    }

    return (
        <div className="max-w-3xl mx-auto pb-12">
            <div className="mb-6">
                <h1 className="text-2xl font-bold">{t('form.title')}</h1>
                <p className="text-muted-foreground">{t('form.description')}</p>
            </div>

            <form action={handleSubmit}>
                <div className="grid gap-6">
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-lg flex items-center gap-2">
                                <Info className="h-5 w-5 text-blue-600" />
                                {t('form.basicInfo')}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="space-y-2">
                                <Label htmlFor="name">{t('form.name')}</Label>
                                <Input id="name" name="name" placeholder={t('form.namePlaceholder')} required />
                            </div>
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="project_id">{t('form.project')}</Label>
                                    <Select name="project_id">
                                        <SelectTrigger>
                                            <SelectValue placeholder={t('table.allProjects')} />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="all">{t('table.allProjects')}</SelectItem>
                                            {projects.map(p => (
                                                <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="type">{t('form.type')}</Label>
                                    <Select name="type" defaultValue={type} onValueChange={setType}>
                                        <SelectTrigger>
                                            <SelectValue placeholder={t('form.select')} />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="Flat %">{t('types.Flat %')}</SelectItem>
                                            <SelectItem value="Flat Amount">{t('types.Flat Amount')}</SelectItem>
                                            <SelectItem value="Tiered">{t('types.Tiered')}</SelectItem>
                                            <SelectItem value="Unit Based %">{t('types.Unit Based %')}</SelectItem>
                                            <SelectItem value="Unit Based Amount">{t('types.Unit Based Amount')}</SelectItem>
                                            <SelectItem value="Project Based %">{t('types.Project Based %')}</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>
                            </div>

                            <div className="bg-slate-50 p-3 rounded-md flex gap-3 text-sm text-slate-600 border border-slate-200">
                                <Info className="h-5 w-5 text-blue-500 shrink-0" />
                                <div>
                                    {/* Using dynamic check but safely since we have default strings if needed */}
                                    {type === 'Flat %' && t('typeDescriptions.Flat %')}
                                    {type === 'Flat Amount' && t('typeDescriptions.Flat Amount')}
                                    {type === 'Tiered' && t('typeDescriptions.Tiered')}
                                    {type === 'Unit Based %' && t('typeDescriptions.Unit Based %')}
                                    {type === 'Unit Based Amount' && t('typeDescriptions.Unit Based Amount')}
                                    {type === 'Project Based %' && t('typeDescriptions.Project Based %')}
                                </div>
                            </div>
                        </CardContent>
                    </Card>

                    <Card>
                        <CardHeader>
                            <CardTitle className="text-lg flex items-center gap-2">
                                <CreditCard className="h-5 w-5 text-green-600" />
                                {t('form.valueConditions')}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="value">
                                        {type === 'Tiered' ? t('form.baseRate') : t('form.standardRate')}
                                    </Label>
                                    <div className="relative">
                                        <Input id="value" name="value" type="number" step="0.01" placeholder="0.00" required />
                                        <div className="absolute right-3 top-2.5 text-xs text-muted-foreground font-bold">
                                            {type.includes('%') || type === 'Tiered' ? '%' : ''}
                                        </div>
                                    </div>
                                    <p className="text-[10px] text-muted-foreground italic">
                                        {type === 'Tiered' ? t('form.baseRateHint') : t('form.standardRateHint')}
                                    </p>
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="currency">{t('form.currency')}</Label>
                                    <Select name="currency" defaultValue="TRY">
                                        <SelectTrigger>
                                            <SelectValue placeholder={t('form.select')} />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="TRY">TRY</SelectItem>
                                            <SelectItem value="USD">USD</SelectItem>
                                            <SelectItem value="EUR">EUR</SelectItem>
                                            <SelectItem value="GBP">GBP</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>
                            </div>
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="payable_stage">{t('form.paymentStage')}</Label>
                                    <Select name="payable_stage" defaultValue="Contract Signed">
                                        <SelectTrigger>
                                            <SelectValue placeholder={t('form.select')} />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="Contract Signed">{t('paymentStages.Contract Signed')}</SelectItem>
                                            <SelectItem value="Payment Received">{t('paymentStages.Payment Received')}</SelectItem>
                                            <SelectItem value="Delivery">{t('paymentStages.Delivery')}</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="payment_terms">{t('form.paymentTerms')}</Label>
                                    <Select name="payment_terms" defaultValue="Net">
                                        <SelectTrigger>
                                            <SelectValue placeholder={t('form.select')} />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="Net">{t('terms.Net')}</SelectItem>
                                            <SelectItem value="Brut">{t('terms.Brut')}</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>
                            </div>
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="start_date">{t('form.startDate')}</Label>
                                    <Input id="start_date" name="start_date" type="date" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="end_date">{t('form.endDate')}</Label>
                                    <Input id="end_date" name="end_date" type="date" />
                                    <p className="text-[10px] text-muted-foreground italic">
                                        {t('form.endDateHint')}
                                    </p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>

                    <div className="flex items-center justify-end gap-3">
                        <Button type="button" variant="outline" onClick={() => router.back()}>{t('form.cancel')}</Button>
                        <Button type="submit" className="bg-blue-600 hover:bg-blue-700 font-bold px-8" disabled={loading}>
                            {loading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : t('form.submit')}
                        </Button>
                    </div>
                </div>
            </form>
        </div>
    )
}
