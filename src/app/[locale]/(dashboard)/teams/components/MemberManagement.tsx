...
export default function MemberManagement({ team, profiles }: MemberManagementProps) {
    ...
    return (
        <Dialog>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{team.name} - {t('title')}</DialogTitle>
                </DialogHeader>
                <div className="space-y-6 py-4">
                            ...
                            <div className="w-full sm:w-[120px]">
                                <Select value={selectedRole} onValueChange={setSelectedRole} disabled={isPending}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="leader">{t('leaderBadge')}</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            <form action={async () => {
                    ...
                    <div className="space-y-2">
                        <h4 className="text-sm font-medium">{t('current')}</h4>
                                ...
                                team.team_members.map((member: any) => (
                                    <div key={member.id} className="flex items-center justify-between p-3">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-xs text-muted-foreground capitalize">{member.role === 'leader' ? t('leaderBadge') : t('memberBadge')}</span>
                                        </div>
                                        <form action={async () => {
                                            const formData = new FormData()
                                            formData.append('team_id', team.id)
