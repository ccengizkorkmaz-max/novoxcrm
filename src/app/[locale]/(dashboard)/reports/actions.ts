...
export async function getCrmStatistics() {
    ...
    // ── 4. Profiller (lead → temsilci adı eşleştirmesi için) ──
    ...
    const newCustomersThisMonth = allCustomers.filter(c => new Date(c.created_at) >= thirtyDaysAgo).length
    const newLeadsThisMonth = allLeads.filter(l => new Date(l.created_at) >= thirtyDaysAgo).length

    // Ortalama Lead → Dönüşüm Süresi (gün)
    const convertedWithDate = allLeads.filter(l => l.status === 'converted' && l.converted_at)
    let avgConversionDays = 0
    if (convertedWithDate.length > 0) {
        const totalDays = convertedWithDate.reduce((sum, l) => {
            const diff = new Date(l.converted_at).getTime() - new Date(l.created_at).getTime()
            return sum + diff / (1000 * 60 * 60 * 24)
        }, 0)
        avgConversionDays = Math.round(totalDays / convertedWithDate.length)
    }
    ...
    // ══════════════════════════════════════════════════════════════
    const countBy = (arr: any[], key: string, labelMap?: Record<string, string>) => {
        const map: Record<string, number> = {}
        arr.forEach(item => {
            const val = item[key] || 'Belirtilmemiş'
    ...
    const genderLabels: Record<string, string> = { 'male': 'Erkek', 'female': 'Kadın', 'Erkek': 'Erkek', 'Kadın': 'Kadın' }
    const genderDist = countBy(allCustomers, 'gender', genderLabels)
    const cityDist = countBy(allCustomers, 'city').slice(0, 15)
    const districtDist = countBy(allCustomers, 'district').slice(0, 15)
    const customerTypeDist = countBy(allCustomers, 'customer_type', { 'individual': 'Bireysel', 'corporate': 'Kurumsal' })

    const demographics = { genderDist, cityDist, districtDist, customerTypeDist }

    // ══════════════════════════════════════════════════════════════
    // Kaynak Analizi
    // ══════════════════════════════════════════════════════════════
    const customerSourceDist = countBy(allCustomers, 'source').slice(0, 15)
    const leadSourceDist = countBy(allLeads, 'source').slice(0, 15)
    const heardFromDist = countBy(allCustomers, 'heard_from')

    const sources = { customerSourceDist, leadSourceDist, heardFromDist }

    // ══════════════════════════════════════════════════════════════
    // Proje Bazlı Lead İstatistikleri
    // ══════════════════════════════════════════════════════════════
    const leadsWithProject = allLeads.filter(l => l.project_id)
    const projectLeadMap: Record<string, { total: number, new: number, contacted: number, qualified: number, converted: number, lost: number }> = {}
    leadsWithProject.forEach(l => {
        const pName = projectMap[l.project_id] || 'Bilinmiyor'
        if (!projectLeadMap[pName]) projectLeadMap[pName] = { total: 0, new: 0, contacted: 0, qualified: 0, converted: 0, lost: 0 }
        projectLeadMap[pName].total++
        const s = l.status as keyof typeof projectLeadMap[string]
        if (s in projectLeadMap[pName]) (projectLeadMap[pName] as any)[s]++
    })
    const projectLeadDist = Object.entries(projectLeadMap)
    ...
    const leadStatusLabels: Record<string, string> = {
        ...
        'converted': 'Dönüştürüldü', 'lost': 'Kaybedildi'
    }
    const leadStatusDist = countBy(allLeads, 'status', leadStatusLabels)
    const leadSubStatusDist = countBy(allLeads.filter(l => l.sub_status), 'sub_status')
    const leadScoreLabels: Record<string, string> = {
        'hot': '🔥 Sıcak (Hot)', 'warm': '🌤️ Ilık (Warm)', 'cold': '❄️ Soğuk (Cold)', 'disqualified': '🚫 Elendi'
    }
    const leadScoreDist = countBy(allLeads.filter(l => l.lead_score), 'lead_score', leadScoreLabels)

    // Temsilci bazlı lead dağılımı
    const repLeadMap: Record<string, number> = {}
    allLeads.filter(l => l.assigned_to).forEach(l => {
        const repName = profileMap[l.assigned_to] || 'Bilinmiyor'
        repLeadMap[repName] = (repLeadMap[repName] || 0) + 1
    })
    const repLeadDist = Object.entries(repLeadMap)
        .map(([name, value]) => ({ name, value }))
        .sort((a, b) => b.value - a.value)

    const leadStats = { leadStatusDist, leadSubStatusDist, leadScoreDist, repLeadDist }

    // ══════════════════════════════════════════════════════════════
    // Zaman Bazlı Trendler (Son 12 Ay)
    // ══════════════════════════════════════════════════════════════
    const monthlyTrend = []
    for (let i = 11; i >= 0; i--) {
        const monthDate = subMonths(now, i)
        ...
        monthlyTrend.push({
            month: label,
            customers: allCustomers.filter(c => new Date(c.created_at) >= mStart && new Date(c.created_at) < mEnd).length,
            leads: allLeads.filter(l => new Date(l.created_at) >= mStart && new Date(l.created_at) < mEnd).length,
            converted: allLeads.filter(l => l.status === 'converted' && l.converted_at && new Date(l.converted_at) >= mStart && new Date(l.converted_at) < mEnd).length
        })
    }
    ...
    // UTM Analizi
    // ══════════════════════════════════════════════════════════════
    const utmSourceDist = countBy(allLeads.filter(l => l.utm_source), 'utm_source').slice(0, 10)
    const utmCampaignDist = countBy(allLeads.filter(l => l.utm_campaign), 'utm_campaign').slice(0, 10)
    const utmMediumDist = countBy(allLeads.filter(l => l.utm_medium), 'utm_medium').slice(0, 10)

    const utm = { utmSourceDist, utmCampaignDist, utmMediumDist }
