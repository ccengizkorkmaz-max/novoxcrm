...
/**
 * Bulk approve all pending web form items from a specific email address.
 * Each item goes through the same approveInboxItem logic (duplicate detection, project matching, etc.)
 */
export async function bulkApproveWebForms(filterEmail?: string) {
    try {
        const supabase = await createClient()
        const { data: { user } } = await supabase.auth.getUser()
        ...
        // Find all pending items from web forms
        let query = supabase
            .from('inbox_items')
            .select('*')
            .eq('tenant_id', profile.tenant_id)
        ...
        // If a filter email is provided, filter by source containing that email
        if (filterEmail) {
            query = query.ilike('source', `%${filterEmail}%`)
        }

        const { data: items, error: fetchError } = await query
        if (fetchError) return { success: false, error: fetchError.message }
        if (!items || items.length === 0) return { success: false, error: 'Aktarılacak web formu bulunamadı.' }
        ...
        for (const item of items) {
            ...
            try {
                ...
                text = parsed.message || parsed.text || item.message
                if (parsed.json) {
                    try { text = JSON.parse(parsed.json).message || text } catch { /* ignore */ }
                }
            } catch { /* not JSON */ }

            // Extract name, email, phone from message
            const nameMatch = text.match(/Ad\s+Soyad:\s*([^:\n\r]+?)(?=\s*(?:E-posta|Telefon|Konu|Proje|$)|[\r\n])/i)
            const emailMatch = text.match(/(?:E-posta Adresi|E-posta):\s*([^:\n\r\s]+?)(?=\s*(?:Ad Soyad|Telefon|Konu|Proje|$)|[\r\n])/i)
            const phoneMatch = text.match(/(?:Telefon Numarası|Telefon No|Telefon|Tel):\s*([\d\s\+\-\(\)\.]+?)(?=\s*(?:Ad\s+Soyad|E-posta|Konu|Proje|Mesaj|$)|[\r\n])/i)

            const overrides = {
                name: nameMatch ? nameMatch[1].trim() : item.name,
                email: emailMatch ? emailMatch[1].trim() : item.email || '',
                phone: phoneMatch ? phoneMatch[1].trim() : item.phone || ''
            }

            const result = await approveInboxItem(item.id, undefined, overrides)
            if (result.success) {
        ...
        console.error('Server error bulk approving web forms:', error)
        return { success: false, error: error.message || 'Unknown error' }
    }
}
