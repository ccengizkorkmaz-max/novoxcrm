        ...
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
                <Card className="max-w-md w-full border-none shadow-xl rounded-3xl text-center p-8">
                    ...
                    <CardTitle className="text-2xl font-bold mb-4">{t('success.title')}</CardTitle>
                    <CardDescription className="text-md mb-8">
                        {t('success.message')}
                    </CardDescription>
                    <Button asChild className="w-full h-12 rounded-xl text-md font-bold">
                        <Link href="/login">{t('success.back')}</Link>
                    </Button>
                </Card>
            </div>
        )
                    ...
                    {step === 1 ? (
                        <form onSubmit={handleRequestCode} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-xs text-slate-400">{t('emailNote')}</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="bg-blue-50 p-4 rounded-2xl mb-6 flex items-start gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-blue-900 leading-none mb-1">{t('verify.title')}</p>
                                    <p className="text-xs text-blue-700">{t('verify.desc')} <strong>{email}</strong></p>
                                </div>
                                <Button
                                    type="button"
                                    variant="ghost"
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="code" className="text-slate-700 font-bold">{t('verify.codeLabel')}</Label>
                                <Input
                                    id="code"
                                    name="code"
                                    required
                            ...
                            <div className="grid gap-6 sm:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="full_name" className="text-slate-700 font-bold">{t('verify.nameLabel')}</Label>
                                    <Input id="full_name" name="full_name" required placeholder={t('verify.nameLabel')} className="h-11 border-slate-100 bg-slate-50/50 rounded-xl focus:bg-white" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="phone" className="text-slate-700 font-bold">{t('verify.phoneLabel')}</Label>
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="company_name" className="text-slate-700 font-bold">{t('verify.companyLabel')}</Label>
                                <Input id="company_name" name="company_name" placeholder={t('verify.companyLabel')} className="h-11 border-slate-100 bg-slate-50/50 rounded-xl focus:bg-white" />
                            </div>

                            <div className="space-y-2">
                                <Label htmlFor="notes" className="text-slate-700 font-bold">{t('verify.notesLabel')}</Label>
                                <Textarea id="notes" name="notes" placeholder={t('verify.notesPlaceholder')} className="min-h-[100px] border-slate-100 bg-slate-50/50 rounded-xl resize-none" />
                            </div>
