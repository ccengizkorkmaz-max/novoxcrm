        ...
        <div className="space-y-6 pb-12">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                    ...
                    <Input placeholder={t('searchPlaceholder')} className="pl-9 rounded-xl border-slate-200 focus:ring-blue-500" />
                </div>
                ...
                {leads && leads.length > 0 ? (
                                ...
                                <Link href={`/broker/leads/${lead.id}`} className="flex flex-col sm:flex-row sm:items-center p-5 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 mt-1">
                                                <span className="text-xs text-slate-500 flex items-center gap-1" suppressHydrationWarning>
                                                    ...
                                                    {new Date(lead.created_at).toLocaleDateString(t('locale') === 'tr' ? 'tr-TR' : 'en-US')}
                                                </span>
                                                {lead.projects?.name && (
                                    ...
                                    <div className="flex items-center justify-between sm:justify-end gap-6 border-t sm:border-t-0 pt-3 sm:pt-0">
                                        <div className="text-right">
                                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">{t('statusTitle')}</p>
                                        ...
                                        {(lead.budget_min || lead.budget_max) && (
                                            <div className="text-right hidden sm:block min-w-[100px]">
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">{t('budgetTitle')}</p>
                                                <p className="text-sm font-bold text-slate-700 flex items-center justify-end gap-1">
                                                    {lead.budget_max ? (t('locale') === 'tr' ? `${lead.budget_max.toLocaleString('tr-TR')} ₺` : `$${lead.budget_max.toLocaleString('en-US')}`) : t('budgetUndefined')}
                                                </p>
                                            </div>
                                        )}
                    ...
                    <Card className="border-dashed border-2 border-slate-200 bg-transparent rounded-2xl">
                        <CardContent className="p-12 text-center">
                            ...
                            <h3 className="text-lg font-bold text-slate-900">{t('emptyLeads')}</h3>
                            <p className="text-slate-500 text-sm mt-1 max-w-xs mx-auto">{t('emptyLeadsDesc')}</p>
