...
// 2. Prepare Batch 4 URLs
// We will send 81 cities + 19 comparisons for BOTH domains = 200 URLs
...
console.log(`\n📋 BATCH 4 - URLs to submit (${urlsToSubmit.length}):`);
...
async function submitUrls() {
    ...
    for (const url of urlsToSubmit) {
        ...
        // Small delay to prevent rate limit (100 req per minute is standard, so 600ms is safe)
        await new Promise(resolve => setTimeout(resolve, 600));
    }

    console.log(`\n${'='.repeat(60)}`);
    console.log(`📊 BATCH 4 RESULT: ${successCount} successful / ${failCount} failed / ${urlsToSubmit.length} total`);
